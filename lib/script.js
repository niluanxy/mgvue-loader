if (module.hot) {
    ...
    if (typeof _NEWCOM_.data != "function") {
        var baseData = _NEWCOM_.data;

        _NEWCOM_.data = function() {
        ...
        if (oldObj.template != _NEWCOM_.template) {
            ...
            for(var i=0; i<arrays.length; i++) {
                var $view = arrays[i], $scope = $view.scope,
                    ...
                    name = $scope.$$name, render;

                // 说明组件是 view 模式调用
                if ((render = $scope.$$render)) {
                    ...
                    $el.parentNode.insertBefore($el.nextSibling, dom);
